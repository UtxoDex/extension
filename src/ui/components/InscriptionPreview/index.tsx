...
export default function InscriptionPreview({ data, onClick, preset }: InscriptionProps) {
  ...
  return (
    <Column
      gap="zero"
      onClick={onClick}
      style={Object.assign({ position: 'relative' }, $containerPresets[preset], {
        borderRadius: '10px',
        overflow: 'hidden',
      ...
      <Column px="md" py="sm" gap="zero" bg="bg4" full>
        <Text text={numberStr} color="gold" size="xs" />
        {/* size= {$timePresets[preset] as any} */}
        {isUnconfirmed == false && <Text text={time} preset="sub" size="xs" />}
      </Column>
    </Column>
  );
}
