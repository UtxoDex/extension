...
function SignTxDetails({ txInfo, type, rawTxInfo }: { txInfo: TxInfo; rawTxInfo?: RawTxInfo; type: TxType }) {
  ...
  if (type === TxType.SIGN_TX) {
    return (
      <Column gap="lg">
        <Text text="Send Remaining Balance" preset="title-bold" textCenter mt="lg" />
        <Row justifyCenter>
...
export default function SignPsbt({
  ...
  return (
    <Layout>
      ...
      <Footer>
        <Row full>
          <Button preset="default" text="Cancel" onClick={handleCancel} full />
          <Button
            preset="primary"
            text={type == TxType.SIGN_TX ? 'Confirm' : 'Confirm & Send'}
